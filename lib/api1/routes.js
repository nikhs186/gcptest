 ...
 */
router.post("/cloudinary/upload/file", upload.single("file"), async (req, res) => {
    try {
        if (!req.file) {
        ...
        const result = await (0, cloudinaryUpload_1.uploadBufferToCloudinary)(req.file.buffer, maintainRatio, optimizeImage);
        return res.status(200).json(result);
    }
    catch (error) {
        const errorMessage = error instanceof Error ? error.message : JSON.stringify(error);
