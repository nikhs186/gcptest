...
import {DealFilters} from "./schema";
 ...
 */
export const parseDealFilters = (req: Request): DealFilters & {hasNoFilter: boolean} => {
  const query = req.query;
  const filters: DealFilters = {};
  let dataFilters: DealFilters = {};
  ...
  if (query.data) {
    try {
      ...
      if (dataValue && typeof dataValue === "object" && !Array.isArray(dataValue)) {
        ...
        const parsedFilters = parseFiltersFromObject(dataValue as Record<string, unknown>);
        ...
        hasDataFilters = Object.keys(parsedFilters).some((key) => {
          ...
          const value = parsedFilters[key as keyof typeof parsedFilters];
  ...
  const merged = {
    ...dataFilters,
    ...filters, // Individual query params come last, so they override data field params
  };
  ...
  const ALL_FILTER_FIELDS: Array<keyof DealFilters> = [
    "deal_id",
    "dealership_car_id",
    "dealership_id",
    "internal_label",
  ...
  return {
    ...merged,
    hasNoFilter,
  } as DealFilters & {hasNoFilter: boolean};
};
 ...
 */
const parseFiltersFromObject = (dataObj: Record<string, unknown>): DealFilters => {
  const filters: DealFilters = {};
  ...
  if (dataObj.location !== undefined) {
    try {
      if (dataObj.location && typeof dataObj.location === "object" && !Array.isArray(dataObj.location)) {
        const loc = dataObj.location as Record<string, unknown>;
        // Support lat, latitude, lng, long, longitude
 ...
 */
const parseNumberArrayFromValue = (value: unknown): number[] | undefined => {
  if (!value) {
    return undefined;
  }
 ...
 */
const parseStringArrayFromValue = (value: unknown): string[] | undefined => {
  if (!value) {
    return undefined;
  }
 ...
 */
const parseBatterycapArray = (value: unknown): (number | null)[] | undefined => {
  if (!value) {
    return undefined;
  }
 ...
 * @return {(number | null)[] | undefined} Parsed batterycap array with null support.
 */
const parseBatterycapArrayFromValue = (value: unknown): (number | null)[] | undefined => {
  if (!value) {
    return undefined;
  }
