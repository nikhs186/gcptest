import {executeQuery, executePlanetScaleQuery} from "./pg";
import {executeClickHouseQuery} from "./clickhouse";
import {DealWithCarDetails, DealFilters, PaginatedDealsResult, CarMedia} from "./schema";
import {getFavoritedDealIds} from "./auth";

/** [min, 0] with min > 0 means minimum-only (e.g. deposit >= 1000), not a closed range. */
const shouldApplyRangeMax = (min: number, max: number): boolean => {
  return !(max === 0 && min > 0);
};

/** Deposit-only: [0, 0] means exactly zero deposit. */
const isDepositExactZero = (values: number[]): boolean => {
 ...
 */
const buildWhereClause = (filters: DealFilters, favoritedDealIds: string[] | null = null) => {
 ...
 */
const buildClickHouseWhereClause = (
  filters: DealFilters,
  favoritedDealIds: string[] | null = null,
 ...
 */
const buildClickHouseOrderBy = (sort: string | null | undefined): string => {
  ...
  let primarySort: string | null = null;
  switch (sort) {
 ...
 * @return {string} ORDER BY clause string.
 */
const buildPlanetScaleOrderBy = (sort: string | null | undefined): string => {
 ...
 */
const buildPlanetScaleWhereClause = (
  filters: DealFilters,
  favoritedDealIds: string[] | null = null,
  ...
  const inList = (values: unknown[]): string => {
  ...
  if (filters.inclusion && Array.isArray(filters.inclusion) && filters.inclusion.length > 0) {
    const or = filters.inclusion.map((inc) =>
      `d.inclusion @> ${p(JSON.stringify([String(inc)]))}::jsonb`
    );
  ...
  if (filters.make && Array.isArray(filters.make) && filters.make.length > 0) {
    const or = filters.make.map((id) =>
      `(dc.make->>'id')::integer = ${p(Number(id))}`
    );
  ...
  if (filters.model && Array.isArray(filters.model) && filters.model.length > 0) {
    const or = filters.model.map((id) =>
      `(dc.model->>'id')::integer = ${p(Number(id))}`
    );
    conditions.push(`(${or.join(" OR ")})`);
  }
  ...
  if (filters.spec && Array.isArray(filters.spec) && filters.spec.length > 0) {
    const or = filters.spec.map((s) =>
      `dc.spec @> ${p(JSON.stringify([String(s)]))}::jsonb`
    );
 ...
 */
async function getPlanetScaleDeals(
  filters: DealFilters,
  consumerId: string | null,
  favoritedDealIds: string[] | null,
  page: number,
  perPage: number,
  isDebugProfile: boolean,
  debugProfileId: unknown,
  ...
  const countQuery = `
    ...
    ${whereClause}
  `;
  const countResult = await executePlanetScaleQuery(countQuery, [...whereParams]);
  ...
  const result = await executePlanetScaleQuery(dataQuery, [...whereParams, perPage, offset]);

  if (isDebugProfile) {
    console.log("matching-planetscale-fetch-result", {
      debugProfileId,
      rowsReturned: result.rows.length,
      first20Ids: result.rows.slice(0, 20).map((r: Record<string, unknown>) => r.id),
    });
  }
  ...
  const deals: DealWithCarDetails[] = result.rows.map((row: Record<string, unknown>) => {
    const deal: DealWithCarDetails = {
      ...
      dealership_car_id: (row.dealership_car_id as string) || "",
    ...
    if (row.car_id) {
      ...
      const makeName = (row.make_name as string) || carMake?.name;
      const modelName = (row.model_name as string) || carModel?.name;
      ...
      deal.car = {
        ...
        created_at: new Date(),
        dealership_id: (row.car_dealership_id as string) || deal.dealership_id,
        make: makeName,
        make_id: parseNumber(row.make_id) || parseNumber(carMake?.id),
        model: modelName,
        model_id: parseNumber(row.model_id) || parseNumber(carModel?.id),
        model_variant: row.model_variant as string | undefined,
        registration_date: parseDate(row.registration_date),
        fuel: row.fuel as string | undefined,
        transmission: row.transmission as string | undefined,
        color: row.color as string | undefined,
        body: row.body as string | undefined,
        tax_class: row.tax_class as string | undefined,
        ...
        spec: Array.isArray(row.spec) ?
          (row.spec as unknown[]).map((v: unknown) => Number(v)).filter((n) => !isNaN(n)) :
          undefined,
        wheel_drive: row.wheel_drive as string | undefined,
        media: row.media as CarMedia | undefined,
      };
    }
  ...
  if (consumerId && deals.length > 0) {
    favoritedSet = await getFavoritedDealIds(consumerId, deals.map((d) => d.id));
  }

  const items = deals.map((deal) => ({
    ...deal,
    favourite: consumerId ? favoritedSet.has(deal.id) : false,
  }));
...
export const getDeals = async (
  ...
  const debugProfileId = (filters as Record<string, unknown>)._debugProfileId;
  ...
  const hasRadius = hasLocationFilter && filters.location?.radius !== undefined && filters.location.radius > 0;
  ...
  const getDistanceExpression = (latParamIndex: number, lngParamIndex: number) => {
    return `CASE
  ...
  if (showUniqueDealerships) {
    ...
    const result = await executeQuery(query, [perPage, offset]);
    ...
    const deals = result.rows.map((row) => {
      ...
      if (carData && typeof carData === "object" && Object.keys(carData).length > 0 && row.car_id) {
        ...
        deal.car = {
          ...
          make: make,
          make_id: parseNumber(row.make_id) || parseNumber((carData.make as {id?: unknown})?.id),
          model: model,
          model_id: parseNumber(row.model_id) || parseNumber((carData.model as {id?: unknown})?.id),
          model_variant: carData.model_variant as string | undefined,
          vin: carData.vin as string | undefined,
          registration_date: parseDate(carData.registration_date),
          registration_number: carData.registration_number as string | undefined,
          status: carData.status as string | undefined,
          sale_status: carData.sale_status as string | undefined,
          hitme: carData.hitme as boolean | undefined,
          ...
          description: carData.description as string | undefined,
          ...
          image: carData.image as string | undefined,
          media: carData.media as CarMedia | undefined,
          internal_label: carData.internal_label as string | undefined,
        };
      }
      ...
      const requestCount = (row as Record<string, unknown>).request_count;
    ...
    if (useClickHouse) {
      const {
        whereClause: clickHouseWhere,
        params: clickHouseParams,
      } = buildClickHouseWhereClause(filters, favoritedDealIds);
      ...
      const clickHouseResult = await executeClickHouseQuery(clickHouseQuery, clickHouseParams);

      const parseClickHouseJsonObject = (value: unknown): Record<string, unknown> | null => {
      ...
      const deals = clickHouseResult.rows.map((row: unknown) => {
        ...
        const dealId = (r.id ?? r["d.id"]) as string | undefined;
      ...
      if (deals.length === perPage) {
        ...
        try {
          const countQuery = `
            ...
            ${clickHouseWhere}
          `;
          const countResult = await executeClickHouseQuery(countQuery, clickHouseParams);
      ...
      // Check favorites
      let favoritedDealIdsSet = new Set<string>();
      if (consumerId && parsedDeals.length > 0) {
        const dealIds = parsedDeals.map((d) => d.id);
        favoritedDealIdsSet = await getFavoritedDealIds(consumerId, dealIds);
      }

      const items = parsedDeals.map((deal) => ({
        ...deal,
        favourite: consumerId ? favoritedDealIdsSet.has(deal.id) : false,
      }));
    ...
    const result = await executeQuery(query, [...mainParams, perPage, offset]);
    ...
    const deals = result.rows.map((row) => {
      ...
      if (hasLocationFilter) {
        ...
        const distance = (row as Record<string, unknown>).distance;
